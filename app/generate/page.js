...
export default function Generate() {
  ...
  // Renamed state variable to avoid confusion with the setter function.
  ...
  const saveFlashcards = async () => {
    ...
    try {
      ...
      let collections = userDocSnap.exists()
        ? userDocSnap.data().flashcards || []
        : [];

      if (collections.find((f) => f.name === flashcardSetName)) {
        alert('Flashcard collection with the same name already exists.');
        return;
      } else {
        collections.push({ name: flashcardSetName });
        batch.set(userDocRef, { flashcards: collections }, { merge: true });
      }

      const colRef = collection(userDocRef, flashcardSetName);
